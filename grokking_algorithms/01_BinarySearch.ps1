function Search-BinaryCycle  # двоичный поиск, исправленный книжный вариант (не задаётся лишний вопрос, когда остаётся одно число)
...
function Search-BinaryRecurseSoft  # двоичный поиск, рекурсивный алгоритм, простая версия правил
{<# простая версия правил означает, что
        - задаётся вопрос про число (серединку)
        - даются ответы "больше" / "меньше" / "угадал"
    ...
    if ($step -eq 0)  # БАЗОВАЯ ЧАСТЬ ФУНКЦИИ: проверим игру на честность, если ок - определяем границы и делаем первый шаг (рекурсивный вызов)
    {
        if ($target -in $lst)  # игра ок: загадано число из списка
        {
            ...
            return (Search-BinaryRecurseSoft $lst $target $step $l $h)
        }
...
function Search-BinaryRecurseHard  # двоичный поиск, рекурсивный алгоритм, сложная версия правил
{<# помогает взглянуть на алгоритм со стороны по-битного определения целевого числа по одному биту за вопрос
    ...
    if ($step -eq 0)  # БАЗОВАЯ ЧАСТЬ ФУНКЦИИ: проверим игру на честность, если ок - определяем границы и делаем первый шаг (рекурсивный вызов)
    {
        if ($target -in $lst)  # игра ок: загадано число из списка
        {
            ...
            return (Search-BinaryRecurseHard $lst $target $step $l $h)
        }
        else { return -1 }  # нечестная игра, числа нет в списке
    }
...
# странно, но при проверке на больших числах выигрывает рекурсия против цикла
# 32768
# 224712,7304 мс   цикл
# 142719,8852 мс   рекурсия софт
# 164528,1803 мс   рекурсия хард

$lim = 8  # кол-во чисел в сгенерированном списке 0..7
...
Search-BinaryRecurseSoft (0..7) 7
Search-BinaryRecurseHard (0..7) 7

# "Search-BinaryCycle    {2}{3}`treal '{1}' vs '{0}' max" -f $ShiftPredict, (Search-BinaryCycle (0..($lim - 1)) $lim), $lim, " in (0..$($lim - 1))"  # число не из списка
# "`nSearch-BinaryCycle    {2}{3}`treal '{1}' vs '{0}' max" -f $ShiftPredict, (Search-BinaryCycle (0..($lim - 1)) ($lim - 1)), ($lim - 1), " in (0..$($lim - 1))"

$ht0 = @{} ; (Measure-Command {
    ($lim - 1)..0 | ForEach-Object {
        $ShiftReal = Search-BinaryCycle (0..($lim - 1)) $_
        
        if ($null -eq $ht0[$ShiftReal])
        {
            $ht0[$ShiftReal] = @($_)
        }
        else
...
# $ht0 | Sort-Object -Property Name

$ht1 = @{} ; (Measure-Command {
    ($lim - 1)..0 | ForEach-Object {
        $ShiftReal = Search-BinaryRecurseSoft (0..($lim - 1)) $_
        
        if ($null -eq $ht1[$ShiftReal])
        {
            $ht1[$ShiftReal] = @($_)
        }
        else
...
# $ht1 | Sort-Object -Property Name

$ht2 = @{} ; (Measure-Command {
    ($lim - 1)..0 | ForEach-Object {
        $ShiftReal = Search-BinaryRecurseHard (0..($lim - 1)) $_
        
        if ($null -eq $ht2[$ShiftReal])
        {
            $ht2[$ShiftReal] = @($_)
        }
        else
