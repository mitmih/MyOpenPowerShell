...
$lim = 7  # последнее число для генерации массива чисел

$ShiftPredict = 0 ; do { $ShiftPredict++ } until ((1 -shl $ShiftPredict) -gt $lim)

"Search-Binary    {2}{3}`treal '{1}' vs '{0}' max" -f $ShiftPredict, (Search-Binary (0..$lim) ($lim * 2)), ($lim * 2), " in (0..$lim)"  # число не из списка


"`nSearch-Binary    {2}{3}`treal '{1}' vs '{0}' max" -f $ShiftPredict, (Search-Binary (0..$lim) $lim), $lim, " in (0..$lim)"
$ht3 = @{} ; <# (Measure-Command { #>
